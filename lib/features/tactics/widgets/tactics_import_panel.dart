...
/// Tactics home screen when no puzzle is active: who the app downloads games
/// for, then the Tactics card — what is in the database and the filters that
/// decide which of it you practise.
///
...
/// * The import/resume status banners. A progress banner on the opposite side
///   of the screen from the progress bar, with its own pause button and a
///   Dismiss that hid live progress, is what made this page confusing. One
///   column starts the analysis and reports on it; this side is what it
///   produced. (The *play* button came back — it sits on the Tactics card with
///   the count it plays. See [_TacticsImportPanelStartCard].)
/// * Two username boxes and the games window, permanently open. Setting up an
///   account happens once, so it is a button onto a dialog ([AccountsCard]);
///   the window is *which games the analysis fetches*, so it moved to the
///   review strip's gear next to the time controls, which decide the same
///   thing. Both are still stated where they matter — the card names the
///   accounts, the strip names the window.
///
...
/// Shared state for [TacticsImportPanel]: what the card mixins read. The
