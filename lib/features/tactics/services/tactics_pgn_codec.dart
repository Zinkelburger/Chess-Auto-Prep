    ...
    try {
      ...
      puzzles.add(
        TacticsPosition(
          ...
          // Old notes are rewritten here, once, so nothing downstream
          // has to know the formats this app used to write.
          mistakeAnalysis: TacticsNote.canonicalize(note ?? ''),
    ...
    try {
      ...
      puzzles.add(
        TacticsPosition(
          ...
          mistakeAnalysis: TacticsNote.canonicalize(comments.join(' ')),
          variationLabel: 'Variation $varNum',
          gameWhite: _cleanName(headers['White']),
          gameBlack: _cleanName(headers['Black']),
          gameResult: headers['Result'] ?? '*',
          gameDate: headers['Date'] ?? '',
