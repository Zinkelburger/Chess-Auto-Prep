...
class ExpectimaxCalculator {
  ...
  int calculate(BuildTree tree) {
    ...
    void visit(BuildTreeNode node) {
      ...
      if (!identical(node, resolved)) {
        ...
        if (node.terminalValue != null || node.children.isEmpty) {
          final value =
              node.terminalValue ??
              (node.hasEngineEval
          ...
          final exact =
              ...
              (node.hasEngineEval &&
                  (!node.historyAware || node.ply >= config.maxPly));
          node.valueLower = exact ? value : 0;
          node.valueUpper = exact ? value : 1;
        } else if (ours) {
          final candidates = eligibleChildren(node);
          final winner = scoreOurMoveChildren(node)!;
  ...
  List<BuildTreeNode> eligibleChildren(BuildTreeNode node) {
