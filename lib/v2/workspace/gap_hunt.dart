...
final class GapHunt extends ChangeNotifier {
  ...
  /// Why the other chapters could not be read at all, when they could not.
  String? get problem => _problem;

  /// The walk over the chapter as it is now; null while one is on its way.
  GapWalk? get currentWalk => !_walking && _problem == null ? _walk : null;
  bool get canNextGap => currentWalk?.gaps.isNotEmpty ?? false;
  ...
  void nextGap() {
    final gaps = currentWalk?.gaps ?? const [];
  ...
  Future<void> _walked(
    ...
    try {
      final around = source == null
          ? const <String, String>{}
          : await _answers.around(source, side);
      if (overtaken()) return;
...
/// a proposal is not an answer, and a chapter that cannot be read answers
/// nothing rather than stopping the others being asked.
final class RepertoireAnswers {
  ...
  Future<Map<String, String>> around(ChapterRef chapter, Side side) async {
    ...
    for (final other in folder.chapters) {
      ...
      // A file that could not be read is asked again next time rather than
      // remembered as answering nothing.
      final answered =
          _read[other] ??
          switch (await (files[other.path] ??= _file(other))) {
            final file? => _read[other] = _answeredIn(other, file),
            null => _Answered.none,
          };
