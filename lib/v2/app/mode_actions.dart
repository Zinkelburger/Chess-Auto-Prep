...
/// then the analysis board's and what can be done to the document,
/// whichever mode opened it, then the card's tabs.
final class ModeActions {
  ...
  /// The entries for the mode on screen now. [board] is the analysis
  /// board's, which ask their questions in the shell's dialogs.
  List<AppAction> now({
    required PaneTabs<WorkspaceTab> tabs,
    required ValueNotifier<bool> editing,
    required List<AppAction> board,
  ...
  List<AppAction> _files() => [
    ...
    if (requests.mode == Mode.repertoires)
      AppAction(
        ...
        shortcut: session.isScratch ? null : 'Ctrl+V',
      )
    else
      AppAction(
        'Close file',
  ...
  List<AppAction> _repertoire(ShellDialogs dialogs) => [
    ...
    AppAction(
      ...
      fill.canStart ? dialogs.generate : null,
      shortcut: 'Ctrl+G',
