...
Fresh Flutter/Stockfish process and app profile per run. Read-only master book.
...
def report(out, results):
    rows = [
        ...
        '40 cp candidate-loss limit, master book with Maia 2200 off-book. '
    ...
    rows += ['', '¹ One observation per case, not a statistical speed claim. '
             ...
             'Full configs, bounds, book coverage, calls, and history-keyed choices are in each stats.json. '
...
def fingerprint(database):
    # Opening a read-only WAL database can create an empty -wal file. That
    # changes no game data. Nonempty journals and the main DB still count.
    files = [Path(database), Path(str(database) + '-wal')]
    return {str(path): [stat.st_size, stat.st_mtime_ns]
            for path in files if path.exists() and (stat := path.stat()).st_size > 0}
...
def main():
    ...
    parser.add_argument('--master-db', default=str(Path.home() / '.local/share/com.example.chess_auto_prep/master_games.db'))
    ...
    if not Path(args.master_db).is_file():
        parser.error('Master database missing; provide --master-db')
    ...
    book_before = fingerprint(args.master_db)
    (out / 'master-book-fingerprint.json').write_text(json.dumps(book_before, indent=2) + '\n')
    binaries = {
    ...
    for index, (name, fen, moves, plies) in enumerate(CASES):
        ...
        for algorithm in (('pure', 'fast') if index % 2 == 0 else ('fast', 'pure')):
            ...
            definitions = dict(ALGO=algorithm, OUT=str(run), MAX_PLY=plies,
                               ...
                               MASTER_DB=str(Path(args.master_db).resolve()), START_MOVES=moves)
            if fen:
                definitions['FEN'] = fen
            ...
            if fingerprint(args.master_db) != book_before:
                raise SystemExit('Master book changed during comparison; do not compare these runs. Rerun on a stable copy.')
