...
/// A line of [ucis] from the initial position, ours on the even plies,
/// each move worth [value]; the positions are real so decisions are keyed
/// the way a search keys them.
DraftLine line(List<String> sans, {double reach = 1, double value = 0.5}) {
  final tree = lineTree(Fen.initial, sans);
...
void main() {
  ...
  group('writing the draft', () {
    test('every move carries its tokens, the first its reach, and the '
        'chapter reads back with Draft in its heading', () async {
      final kept = line(
      ...
      final chapter = await readChapter(name: 'Main (draft)', text: text);
