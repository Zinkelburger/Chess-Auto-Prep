...
void main() {
  ...
  test('corrupt recovery metadata refuses first read and create', () async {
    ...
    expect(
      await fixture.store.create(other, oneGame('1. e4')),
      isA<IoFailure>(),
    );
    expect(await File(other.path).exists(), isFalse);
    expect(await note.readAsString(), 'not-json');
  });
  ...
  test(
    'unresolved recovery blocks training, scans, migration and import',
    () async {
      ...
      expect(await training.read({flat.path}), isA<ProgressFailed>());
      expect(await training.write(), isA<ProgressFailed>());
      ...
      expect(await chapters.list(), isA<RepertoiresUnreadable>());
      expect(await chapters.deleted(), isA<DeletedUnreadable>());
      expect(
        await StudyDirectory(
          Directory(p.join(fixture.documents.path, 'studies')),
          recovery: fixture.store.recovery,
        ).list(),
        isA<StudiesUnreadable>(),
      );
      ...
      expect(imported, isA<ImportFailed>());
      expect(await File(flat.path).exists(), isTrue);
      expect(
        await Directory(
          p.join(fixture.documents.path, 'pgn_collections'),
        ).exists(),
        isFalse,
      );
      expect(await note.readAsString(), 'not-json');
