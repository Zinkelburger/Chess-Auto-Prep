...
void expectRoundTrip(String game) {
  ...
  expect(
    ...
    isA<RewriteReady>(),
    reason: 'the rewrite gate refused $game',
  );
...
/// The reason the gate gives for refusing to write [read] again, or null
/// when it allows it.
String? refusal(GameRead read) {
  final tree = read.tree;
  if (tree == null) return 'the game has no position';
