...
void main() {
  ...
  test('multi-move tactic: opponent auto-plays after correct user move', () {
    ...
    session.attachPanel(TacticsPanelHooks(applyBoardUpdate: boardUpdates.add));
  ...
  group('handleMoveAttempted routing', () {
    test('routes to onAnalysisMove in analysis mode', () {
      ...
      session.attachPanel(
        TacticsPanelHooks(
          playAnalysisMove: (uci) => analysisMove = uci,
          applyBoardUpdate: boardUpdates.add,
        ),
      );
    ...
    test('validates move and fires board update, advancing the line', () {
      ...
      final boardUpdates = <TacticsBoardUpdate>[];
      session.attachPanel(
        TacticsPanelHooks(applyBoardUpdate: boardUpdates.add),
      );
      ...
      expect(
        ...
        1,
        reason: 'correct move advanced the line',
      );
      expect(session.positionSolved, isTrue);
    });
  ...
  group('session outcomes and recap', () {
    ...
    test('auto-advance past the last puzzle fires onSessionCompleted', () {
      ...
      var completed = false;
      session.attachPanel(
        TacticsPanelHooks(sessionCompleted: () => completed = true),
      );
  ...
  test(
    ...
    () {
      ...
      final session = TacticsSessionController(database: db)
        ..autoAdvance = false;
      session.selectPosition(db.positions.first);
      session.attachPanel(TacticsPanelHooks(applyBoardUpdate: (_) {}));
