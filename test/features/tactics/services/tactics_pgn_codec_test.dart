...
''';
  ...
  group('legacy note migration', () {
    // Notes are rewritten to the current shape at decode, so the upgrade is
    // persisted on the set's next save rather than redone on every render.
    test('a stored legacy note decodes as the canonical format', () {
      final pgn = encodePuzzlesToPgn('Set', [
        _puzzle(
          fen: '6k1/5ppp/8/8/8/8/r7/6K1 b - - 0 1',
          line: ['Ra1+'],
          note:
              'Blunder: Ra3 dropped your eval from +0.5 to -2.1 '
              '(win chance 55% → 21%). Best was Ra1+.',
        ),
      ]).pgn;
      ...
      expect(decoded.errors, isEmpty);
      expect(
        decoded.puzzles.single.mistakeAnalysis,
        'Ra3 +0.5 → -2.1, Ra1+ +0.5',
      );
    });
  });
