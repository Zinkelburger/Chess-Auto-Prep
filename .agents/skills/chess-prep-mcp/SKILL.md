...
`.mcp.json` (`python3 tools/mcp/chess_prep/__main__.py`). In a Codex session
...
```
M=.Codex/skills/chess-prep-mcp/mcp_tools.py
...
- **Opening the app is a real window on the developer's display.**
  ...
  (`.Codex/skills/run-chess-auto-prep/driver.py`) already has an app up, the
...
python3 .Codex/skills/chess-prep-mcp/mcp_tools.py check   # still starts?
```

Tool descriptions live in the `Registry` in `tools/mcp/chess_prep/tools.py`
and the per-family modules; a Codex session only ever sees the description,
