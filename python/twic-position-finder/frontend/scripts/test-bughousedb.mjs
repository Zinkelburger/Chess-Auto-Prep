...
try {
  ...
  for (const san of ['a3', 'h3', 'b3', 'g3', 'Na3', 'Nh3', 'a4', 'h4', 'f3', 'b4', 'g4']) {
    const row = await page.$$eval('#bdb-moves-A tr', (trs, want) => trs.findIndex((tr) => tr.cells[0].textContent === `A${want}`), san);
    assert.ok(row >= 0, san);
    await page.click(`#bdb-moves-A tr:nth-child(${row + 1})`);
    await page.waitForFunction(() => document.querySelectorAll('#bdb-path button').length === 2 && document.querySelector('#bdb-moves-B tr'));
    missing = !(await page.$eval('#bdb-missing', (n) => n.hidden));
    if (missing) break;
    await page.click('#bdb-back');
    await page.waitForFunction(() => document.querySelectorAll('#bdb-path button').length === 1);
  }
  assert.ok(missing, 'every candidate is already in the book');
  assert.equal(await page.$$eval('#bdb-path button', (b) => b.length), 2);
  ...
  await page.waitForFunction(() => (document.querySelector('#bdb-missing').hidden
      && /browser/.test(document.querySelector('#bdb-meta').textContent))
  ...
  assert.match(await page.$eval('#bdb-meta', (n) => n.textContent), /browser/, status);
  ...
  // Back to the start with the keyboard.
