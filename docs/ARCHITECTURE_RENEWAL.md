...
| Implemented; verification in progress | Builder directly composes its existing `InteractivePgnEditor` | Deleted `PgnWithAnalysisPane`, `EditMainZone` and the unreachable `RepertoireAnalysisDock`. Direct bindings retain clipboard, title, annotation, branch undo and editor identity. Existing workspace/document/board owners remain; the complete changed production scope shrinks by 763 handwritten lines. This bounded deletion leaves whole Builder renewal Partial. |
