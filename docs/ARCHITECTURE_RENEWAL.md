...
| 12 | **Engine tournament and Bughouse lab** on the shared supervisor. | Screenshot | Partial 2026-09-23, the Bughouse half (owner has not yet seen the screenshot): 4.5k lib lines against the old lab's 11.8k, 101 tests. Built from the web pages, spec in `docs/v2/features/bughouse-lab.md`: `chess/bughouse/` (two-board table, per-board line, setup boxes, Hivemind's scale and joint moves; keys, SAN and UCI checked against the Python tools on a generated fixture), `storage/bughouse_books.dart` (Hivemind book and FICS archive, read only), `engines/hivemind_*.dart` on the shared supervisor (install checked against the manifest before every launch), `features/bughouse/` (the table, the scores from the book or a live search, Analyze, the archive, the one screen). Screenshot: the start from the book; a live search filling in; Analyze. Not built: the engine tournament panel (bughouse matches), the Engine tournament mode |
