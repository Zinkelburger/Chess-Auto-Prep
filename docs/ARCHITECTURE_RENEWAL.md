...
| Study appearance cutover (`6270a265`); whole Study remains Partial | Study uses the app’s existing appearance owner throughout its PGN, engine, dialogs and setup controls | Removes Study’s forced-dark boundary, the old PGN-style path and 23 legacy-theme consumers (205→182). Existing cached rows and floating preview follow appearance without losing drafts, cursor or analysis lifetime. Complete 45-file scope 15,127→15,404 (+277 handwritten); generated +2,199 separately: capability completion, not a size-reduction pass. Independent source review, focused controls, seven Linux native cases and inspected actual-app screenshots pass; final combined widget validation is recorded in the evidence. Namespace source identity/training references, controller diagnostics, whole-feature simplicity and host/accessibility gates remain open. |
