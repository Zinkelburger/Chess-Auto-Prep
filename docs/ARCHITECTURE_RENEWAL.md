...
| 13 | **Services.** Settings screen, Lichess and chess.com accounts, updates, diagnostics (**Open log folder**, copy diagnostics); theme polish; Widgetbook. | Screenshot | Partial 2026-09-21: the settings store and dialog. One `Settings` value in `storage/settings.dart`, one writer (`SettingsStore`, `settings.json` in the support folder, a failed read named on the page and kept as defaults); a 640×300 dialog (the gear at the right end of the top bar, Ctrl+,) with a list of five places — Look, Engine, Files, Accounts, App — and the chosen place's rows, one line each, searchable across places. Rows today: board coordinates; engine cores, memory and lines (the engine restarts for cores or memory, re-searches for lines); copy files from outside Documents on open; the Lichess token (the old app's key); Open log folder. Owner's rule for the page: a row only when two people want different values and the app cannot tell; a mode's own knobs stay in the mode. Not built: theme, figurines, chess.com, updates, diagnostics copy, Widgetbook |
