...
| Complete bounded cut; Builder remains Partial | Builder directly composes its existing `InteractivePgnEditor` | Deleted `PgnWithAnalysisPane`, `EditMainZone` and the unreachable `RepertoireAnalysisDock`. Direct bindings retain clipboard, title, annotation, branch undo and editor identity. Existing workspace/document/board owners remain; the complete changed production scope shrinks by 763 handwritten lines. Independent review, 74 focused tests, both native annotation/undo journeys, analyze/lint and compact/wide screenshots pass. Whole Builder renewal remains Partial. |
