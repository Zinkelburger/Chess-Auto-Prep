...
blocks authentication changes until a successful read retry. Shared preference
...
public requests can deliberately fall back to anonymous access. Login diagnostics
redact transport and parse payloads. Existing plaintext v1 keys remain unchanged:
...
| H6 | H4, H5 | All existing modes: focus/shortcuts/navigation/close, settings, credentials, diagnostics and integrity checks | The complete cross-mode sequence below passes with real disposable storage, offline/error cases and headless UI checks | In progress: credential read/retry and redacted diagnostics tranche has five reproduced behavior failures and 58 passing focused storage, owner, login and settings-widget tests. Settings startup protection is in a separate checkpoint; integrity checks and complete cross-mode acceptance remain. |
