...
cutover plan comes first; historical checkpoint evidence is retained at the end
and does not authorize new transitional layers.
...
The current work is consolidation: finish existing replacements before widening
into more features. Whole renewal is still Partial.
...
**partial inventory** in the historical execution record.
...
checkpoint sequencing is superseded by the current execution order above;
