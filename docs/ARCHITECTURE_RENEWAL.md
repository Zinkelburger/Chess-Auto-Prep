...
| In progress: Training completion | Existing session owns finish → persistence → tally → advance; results widget only renders and sends commands | Delete widget post-frame rating/advance scheduling and the forwarded state/callback bag. Pending or failed completion cannot advance manually or automatically; retry resumes the existing captured write. Verify absent/remounted UI, repeated commands and source replacement. |
| In progress: Viewer movement | Existing reading owner dispatches navigation to the explicitly selected reader or its existing game/tree/Solitaire owners | Delete `PgnPaneRouter` and duplicate supplier routing. Preserve reference-reader isolation, fullscreen's primary-reader policy, autoplay cancellation and filter-position editing. Count the complete screen/parts/reading scope. |
| Catalog chapter workflow remains open | One shared chapter mutation boundary must replace picker/Builder/outline duplication | Immediately repair the confirmed competing-create and disposed-picker defects. A later cutover must retire `ChapterStore` and legacy picker ownership with all callers; preserve progressive course reads and explicit retry. The temporary catalog adapter and legacy Library organizer still block whole-catalog completion. |
