...
| Next: finish existing migrated workflows | Close catalog, Study, Builder, Viewer and Training end to end: captured namespace mutations and training references; remaining legacy consumers/lifetimes; active screen coordination; and applicable UI, recovery and parity gates. Keep separate dependency-ordered cutovers where each deletes a complete responsibility. | No remaining temporary owner in each declared-complete workflow. Existing extracted code is reused or simplified; do not restart another extraction cycle. |
...
| Tracked scope | Sept 16 `e477dc58` | Review `a6238ff5` | Main `e64320ce` | Main versus Sept 16 |
|---|---:|---:|---:|---:|
| All library Dart | 218,957 / 890 files | 236,991 / 1,055 files | 227,510 / 992 files | +8,553 (+3.9%) |
...
Main is 9,481 lines smaller than the review snapshot; only Viewer among these
four consumers is smaller than September 16. Legacy `services/` still holds
42,626 lines and `widgets/` 49,719. Riverpod's production imports and package
...
composer graph. Builder line-table and Viewer collection-load consolidation
(`e64320ce`) remove a further 105 and 101 lines respectively from their
separate live graphs. Chapter read/create
...
6. **Use the result to choose the next unit.** Keep working designs and remove
   ...
   pattern to another feature. Declare necessary capability or safety additions
   before implementation and account for their growth separately. A failed
   simplification cannot be relabeled afterward to bypass the reduction gate.
   Put detailed evidence in the evidence record, with one current status update
   here.

| Order | Final result | Required removals and evidence |
|---|---|---|
| Builder line-table consolidated (`e64320ce`) | Existing browser owns filtering/sorting/indexes/scrolling and renders its list/header directly | Full 13-file graph 5,324→5,219 (−105 handwritten). Deletes sole-consumer LinesListPanel and private header forwarding: 22+4 inputs, no new owner. Lazy delegates retain their list/widget/index snapshots; sort, filters, coverage, row targets and native layout retain parity. [Review and checks](ARCHITECTURE_RENEWAL_EVIDENCE.md#builder-line-table-and-viewer-collection-load-consolidation). Whole Builder remains Partial. |
| Viewer collection loading consolidated (`e64320ce`) | Existing document owns admission, read/decode/stat and adoption/restoration through the same injected boundaries | Full 14-file graph 5,308→5,207 (−101 handwritten). Deletes the private loader and intermediate result family; moves its single request epoch into the adopting owner. Draft protection, errors, late-result rejection and restoration remain covered through the actual owner. [Review and checks](ARCHITECTURE_RENEWAL_EVIDENCE.md#builder-line-table-and-viewer-collection-load-consolidation). Whole Viewer remains Partial. |
...
| Catalog remaining mutations are open | Captured document identity, recoverable namespace changes and training-reference ownership must agree before rename/move/delete can be complete | Splitter safety is repaired (`7e09cf81`, +306 handwritten lines). Manual replacement deletion is repaired in `c92ee2e6` ([evidence](ARCHITECTURE_RENEWAL_EVIDENCE.md)); baseline regressions still prove incomplete/late training-reference updates. Destination clobber is repaired at the existing native move boundary (`e59c351b`, +19 Dart/C lines); source identity and full relocation remain open. Do not add a rename journal facade before resolving stale training writes and path reuse; the proposed wider rewrite is not admitted. Existing-chapter mutation, line transfers, the temporary catalog adapter and legacy Library organizer still block whole-catalog completion. |
...
Completed cutover details are recorded once in [the evidence record](ARCHITECTURE_RENEWAL_EVIDENCE.md).
The table keeps the latest verified results, unresolved decisions and blocked
completion gates; historical extraction/deletion counts do not authorize new work.
