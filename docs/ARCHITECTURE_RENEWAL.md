...
| Next bounded deletion, after both trials pass | Builder directly composes its existing `InteractivePgnEditor` | Delete `PgnWithAnalysisPane`, `EditMainZone` and the unreachable `RepertoireAnalysisDock`; their sole caller disables the extra toolbar/dock. Preserve clipboard actions, annotations, title, branch undo and editor identity. Keep existing workspace/document/board owners; measure the full caller/deletion scope and review it independently. |
