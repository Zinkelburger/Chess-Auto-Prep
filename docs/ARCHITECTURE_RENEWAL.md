...
| Orphaned Builder layout retired (`2ead1e6c`); direct-editor cutover in progress | Remove unused layout implementations; Builder directly composes its existing `InteractivePgnEditor` | Six unused Edit Context files removed (−1,124 production lines); live Viewer splitter retained, 50 focused tests and independent review pass. Next delete `PgnWithAnalysisPane`, `EditMainZone` and the unreachable `RepertoireAnalysisDock`, preserving clipboard actions, annotations, title, branch undo and editor identity with existing owners. |
