...
| 6a | **PGN Viewer.** Open a PGN file (recent list or the desktop's dialog), list its games, one game at a time on the shared board, annotate through the same saver. | Screenshot | Done 2026-09-21 (owner has not yet seen the screenshot): 19.5k lib lines total, 894 tests; the left column lists the file's games (`n  White – Black  result`) with a search and a `‹ Game n of N ›` counter, a click puts that game on the board without reading the file again, the header names the game, the recent list is the old app's (`pgn_viewer_recent_files`), and a file outside Documents opens to read because the store keeps versions only for files inside it. Not built: paste from the clipboard, filters, sort, the remembered reading position, the My books, Evaluation graph, Tree and Collection tabs, engine review, solitaire, autoplay, fullscreen, ↑/↓ as previous/next game, handoffs from other modes |
