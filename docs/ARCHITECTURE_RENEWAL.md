...
| Repertoire builder | Folded into Repertoires (owner, 2026-09-21): workspace + chapter outline + a Replies tab (Maia shares, gaps, Next gap); generation writes draft chapters, no tool panels |
...
| 7 | **Generation.** Launch from a chapter, progress, cancel, publish against the source revision, results in the side panel; expectimax rewritten from ALGORITHM.md and compared with the old app on the same input. | One real build | Not started; `Fill gaps from here…` is a disabled Actions entry and the draft-chapter mechanics its output needs (`// Draft`, line moves) exist (owner decisions 2026-09-21 in `docs/v2/features/generation.md`). |
