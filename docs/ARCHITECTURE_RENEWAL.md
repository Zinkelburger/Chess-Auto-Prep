...
workspace/scratch recovery with facade deletion, Study import/publication, and
Generation retained-output recovery. Builder closure also requires bounded
...
Do not repeat the present composition pattern across the 17 unfinished features.
...
| Unify composition | Constructors plus Provider throughout catalog/settings/app presentation | Remove Riverpod owners/bindings/dependency and generic StoredGameScope/DisplaySettingsScope lookups in the same cutover; preserve lifecycle, retry, settings and theme tests. Do not wrap Riverpod in Provider. |
| Simplify Viewer consumers and wiring | Screen composes focused views; existing owners perform their commands with explicit inputs and narrowly scoped dependencies | Remove nested owner discovery from leaf widgets, unnecessary supplier networks, UI workflow decisions and raw error routing; retain unified abandonment and native/recovery tests. Close ARCH-03 before Viewer workflow graduation. |
| Consolidate shared document contracts | Each remaining port has a real boundary; pure internal work uses existing concrete functions/classes | Audit all 17 contracts and actual fakes, delete proven redundant seams and callback-only collaborators with their callers, preserve worker/close/decoder failure coverage. Do not promise deletion of useful boundaries to meet a file count. |
