...
| `pgn/viewer_game_model.dart` | Private parsed input/mainline; immutable metadata and cached move snapshots for the viewer widget. Sidelines still use the legacy mutable forest. | `load` detaches input before normalization; `setMainlineComment`/`toggleMainlineNag` accept an expected move identity to reject stale callbacks; annotation adoption preserves cursor/memo identity and rejects a different starting position |
...
| `../chess_core/pgn/pgn_game_view.dart`, `pgn_game_copy.dart` | Detached immutable metadata/move values and iterative parsed-tree adoption | Annotation containers are read-only in views; codec exports allocate independent mutable lists. Copying preserves deep trees and sibling order. |
