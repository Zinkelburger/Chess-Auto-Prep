...
| `python/bughouse-web/` | Independent bounded Hivemind HTTP service for the Astro `/bughouse` page (`frontend/src/bughouse/app.ts`). Reuses `tools/mcp/bughouse` for two-board legality, cross-board reserves and calibrated joint analysis. One disposable process group per search, one concurrent search, per-client/body/budget limits; no app databases. Docker/Compose and nginx examples support the existing API host or a standalone subdomain. Source is implemented; production deployment is separate. [Hosting and verification](../python/bughouse-web/README.md). |
