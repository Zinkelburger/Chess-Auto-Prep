...
- **Top bar**: the left title holds the current material, breadcrumb and contextual status. The right controls are **Actions ▾ → separator → View selector → settings gear**. The shared mode switcher owns the separator and spacing, with the current mode name as its anchor; labelled actions have at least 44px click targets. This separates screen operations from app navigation consistently across views. Actions open on hover or click and use named groups, with no settings-only ellipsis. Shared Actions and view menus use 32px minimum rows, 13px labels, a 240px minimum width and 16px horizontal insets; section headings or small gaps separate groups without divider lines. Both Actions and the view selector support keyboard navigation, Escape and outside-click dismissal. Player analysis retains download refresh beside its subtitle; PGN Viewer retains collection filters on the left.
- **Settings**: Every `AppSettingsButton(mode: ...)` opens the same persistent settings route at that view. Its sidebar expands the selected view into chapters, with one chapter controlling the right pane; global categories remain accessible. Trainer chapters are Session, Learning, Playback and Material. Tactics separates Session, Puzzle selection, Game downloads and Review performance; its inline Filters and Analysis buttons deep-link here too. Review changes use Apply; session and trainer preferences save automatically. PGN Viewer separates Analysis, Playback and Board and moves. Analysis-panel gears open the shared Engine → Engine analysis / Analysis panels chapters. Narrow windows use searchable view and chapter pickers. `ViewSettingsRegistry` connects mounted view-owned builders; unmounted views initialise underneath the open route, respecting the generation lock. Global account drafts survive category navigation.
...
and **Export**. Actions opens on hover or click; its Tree and Export submenus
also open on hover or click, with keyboard navigation and Escape dismissal.
**Tree → Collection tree** explores the loaded, filtered games;
**Tree → Database explorer** opens the Lichess / Masters / local TWIC source
selector. The tabs use those same names. Export offers PGN, SCID, Copy Game PGN
...
Collection tree or Database explorer through its Tree submenu. The strip appears only with
...
- **Editing indicator and annotation panel**: Orange borders and a prominent **Editing PGN / Finish editing** bar distinguish editing from reading. The bottom panel shows bright NAG buttons and an editable comment field for the current move. File-backed games say **Changes to the file are saved**; pasted games explain how to keep changes. Toggling a NAG replaces any existing move-quality NAG.
...
| `tactics_session_settings.dart` | `TacticsSessionSettings` — order (`newestFirst`/`leastReviewed`/`worstSuccessRate`/`random`), `mistakeTypes` filter, `includeOneStar` toggle, `acceptAlternatives` (ask Stockfish about a non-matching move before calling it wrong; off by default); `accepts(pos)` for session filtering |
...
| `pgn/pgn_game_filter_workspace.dart` | Full-width **Filter games** workspace tab opened from **Actions → Filter games**. A light surface, one Add condition control, and collapsed Position/Move sequence sections keep the initial view simple. Draft conditions and results survive tab switches; a changed collection starts a fresh draft from its saved filters. Conditions use editable field/rule/value rows with collection suggestions. Position accepts FEN/moves, current-board capture, and the embedded board editor. Exact position search includes all pieces, side to move, castling and en passant (mainline and variations). Live results have no separate hidden search; Show games or a result click applies the draft and returns to Game. Debounces by 300ms and hides stale results; invalid input and unfinished board setup block applying. Optional `fenIndex` accelerates position matching. |
