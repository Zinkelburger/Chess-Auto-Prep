...
- **Editing indicator and annotation panel**: A neutral **Editing PGN / Finish editing** bar identifies editing. The shared Notes panel uses a plain comment field without a move-specific placeholder; the target move is labelled above it. NAG buttons retain their move-quality colors. The PGN bar stays visible outside edit mode. With autosave enabled, it only reports pending saves or errors; there is no idle saved-status label or redundant **Save** button. Manual saving shows **Save** and reports **Unsaved changes** when needed. Failed autosaves with pending edits also expose **Save** to retry. Pasted games offer **Save as…** to create a backing PGN file. Toggling a NAG replaces any existing move-quality NAG.
