...
with tempfile.TemporaryDirectory(prefix='rolling-cli-') as temp:
    base = Path(temp) / 'tree'
    command = [str(root / 'bin/tree_builder'), '--search', 'rolling',
    ...
    assert '[Search "Rolling 4-ply (approximate)"]' in pgn
    ...
    print('Rolling CLI: selection, saved mode, PGN labels and mode-preserving export passed.')
