...
static char* tree_to_json_internal(const Tree *tree, const SerializationOptions *opts) {
    ...
    if (config) {
        ...
        cJSON_AddNumberToObject(config, "max_eval_loss_cp", tree->config.max_eval_loss_cp);
        cJSON_AddBoolToObject(config, "use_master_games", tree->config.use_masters && !tree->config.maia_only);
        cJSON_AddBoolToObject(config, "maia_only", tree->config.maia_only);
...
static TreeNode* cjson_to_node(cJSON *obj, TreeNode *parent,
    ...
    /* Parse engine eval */
    node->is_repertoire_move=cJSON_IsTrue(cJSON_GetObjectItem(obj,"is_repertoire_move"));
    ...
    cJSON *lower=cJSON_GetObjectItem(obj,"value_lower"), *upper=cJSON_GetObjectItem(obj,"value_upper");
    node->value_lower=cJSON_IsNumber(lower)?lower->valuedouble:0;
    node->value_upper=cJSON_IsNumber(upper)?upper->valuedouble:1;
...
Tree* tree_load_from_buffer(const char *buffer, size_t size) {
    ...
    if (config) {
        ...
        if(cJSON_IsBool(item)) tree->config.play_as_white=cJSON_IsTrue(item);
        item=cJSON_GetObjectItem(config,"eval_depth");
        if(cJSON_IsNumber(item)) tree->config.eval_depth=item->valueint;
        item=cJSON_GetObjectItem(config,"max_eval_loss_cp");
        if(cJSON_IsNumber(item)) tree->config.max_eval_loss_cp=item->valueint;
        item=cJSON_GetObjectItem(config,"use_master_games");
        if(cJSON_IsBool(item)) {tree->config.use_masters=cJSON_IsTrue(item);tree->config.maia_only=!cJSON_IsTrue(item);}
        item=cJSON_GetObjectItem(config,"maia_only");
        if(cJSON_IsBool(item)) tree->config.maia_only=cJSON_IsTrue(item);
        item=cJSON_GetObjectItem(config,"maia_elo");
        if(cJSON_IsNumber(item)) tree->config.maia_elo=item->valueint;
