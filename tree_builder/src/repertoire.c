...
static void build_repertoire_recursive(TreeNode *node, Tree *tree,
    ...
    if (is_our_move) {
        ...
        if (winner.child && *num_moves < max_moves) {
            ...
            /* Traverse every candidate move's subtree so repertoire moves
             * are recorded for all our-move FENs extract_lines may reach
    ...
    while (stack_top > 0 && num_lines < max_lines) {
        ...
        if (is_our_move) {
            ...
            if (!selected) {
                TreeNode *resolved = resolve_transposition(node);
                ScoredChild winner;
                score_our_move_children(resolved, config, &winner);
                selected = winner.child;
            }
...
RepertoireResult* generate_repertoire(Tree *tree, RepertoireDB *db,
    ...
    if (tree->root->history_aware) printf("  %s: %s; policy expected-score bounds [%.6f, %.6f]\n",
        tree->config.rolling_search ? "Fast (4-ply, approximate)" : "Pure",
...
bool export_traps_pgn(const TrapLineInfo *lines, int num_lines,
    ...
    for (int i = 0; i < num_lines; i++) {
        ...
        fprintf(f, "[Search \"%s\"]\n", config->rolling_search ? "Fast (4-ply, approximate)" : "Pure finite horizon");
        {
...
static int line_importance_cmp_desc(const void *a, const void *b) {
    ...
    if (lb->probability < la->probability) return -1;
    if (lb->num_moves > la->num_moves) return 1;
    if (lb->num_moves < la->num_moves) return -1;
    return 0;
}
...
bool repertoire_export_pgn(const RepertoireResult *result,
    ...
    /* Sort by cumulative probability (desc), then line length (desc). */
    ...
    if (config && config->build_time_seconds > 0) {
        fprintf(f,
                ...
                config->build_max_depth > 0
                    ? config->build_max_depth
    ...
    for (int i = 0; i < result->num_lines; i++) {
        ...
        if (!config || config->rank_lines_by_importance) {
            fprintf(f, ", [%%cumProb %.3f%%]", line->probability * 100.0);
        }
        fprintf(f, "}\n");
